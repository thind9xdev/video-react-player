...
describe('ForwardReplayControl', () => {
  ...
  it('should render with "video-react-player-control video-react-player-button" class', () => {
    const Forwardontrol = ForwardReplayControl('forward');
    const wrapper = shallow(<Forwardontrol actions={{}} player={{}} />);
    expect(wrapper.hasClass('video-react-player-control video-react-player-button')).toBe(
      true
    );
  });
});
