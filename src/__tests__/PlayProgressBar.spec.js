...
describe('PlayProgressBar', () => {
  it('should render with "div" tag', () => {
    ...
    expect(wrapper.find('div.video-react-player-play-progress').length).toBe(1);
  });

  it('should render with "video-react-player-play-progress" class', () => {
    const wrapper = shallow(
      <PlayProgressBar currentTime={10} duration={1000} />
    );
    expect(wrapper.hasClass('video-react-player-play-progress')).toBe(true);
  });
